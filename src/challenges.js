...
function multiplyBy(arr, number) {
  let multiplyedArr = [];
  arr.forEach((value) => {
    multiplyedArr.push(value * number);
  });

  return multiplyedArr;
}
...
// *****for continue not completed alredy *****
function filterOut(arrOriginal, arrtoRemove) {
  let newRemovedArr = arrOriginal;
  for (let i = 0; i < arrOriginal.length; i++) {
    for (let j = 0; j < arrtoRemove.length; j++) {
      if (arrOriginal[i] === arrtoRemove[j]) {
        newRemovedArr.shift(i);
      }
    }
  }
  if (arrOriginal == []) {
  ...
  console.log(arrOriginal, newRemovedArr);
  return newRemovedArr;
}
